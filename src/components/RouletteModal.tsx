...
export const RouletteModal: React.FC<RouletteModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-md animate-fade-in overflow-y-auto">
      <div className="relative w-full max-w-4xl bg-[var(--bg-panel)] border-2 border-[var(--neon-cyan)] rounded-2xl shadow-neon-cyan overflow-hidden crt-monitor my-6">
        {/* Header Bar */}
        <div className="flex flex-wrap items-center justify-between px-6 py-3 bg-[var(--bg-brick)] border-b border-[var(--neon-cyan)]/40 gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>▶ Reproductor VidKing</span>
            </button>

            <button
              onClick={onClose}
              className="p-1 ml-2 text-[var(--ink-muted)] hover:text-[var(--neon-magenta)] transition-colors rounded"
            >
              <X className="w-6 h-6" />
            </button>
          </div>
        ...
        {activeView === 'player' ? (
          ...
          <div className="p-4 sm:p-6 space-y-4">
            <div className="relative aspect-video w-full rounded-xl overflow-hidden border-2 border-[var(--neon-cyan)] shadow-neon-cyan bg-black">
              <iframe
                src={vidkingEmbedUrl}
            ...
            <div className="flex items-center justify-between text-xs font-mono text-[var(--ink-muted)] px-1">
              <span className="flex items-center gap-1.5 text-[var(--neon-cyan)] font-bold">
                <Tv className="w-4 h-4" /> Reproductor Terminal VidKing Activo
              </span>
              <button
                onClick={() => setActiveView('details')}
                className="text-[var(--neon-amber)] hover:underline"
              >
                Volver a Detalles
              </button>
            </div>
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
            ...
            <div className="relative aspect-[2/3] w-full max-w-xs mx-auto rounded-lg overflow-hidden border border-[var(--neon-cyan)]/40 shadow-md group">
              ...
              <button
                onClick={() => setActiveView('player')}
                className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 text-white font-mono text-xs font-bold"
              >
